...
const template = require("./feature-paginated-list-embed.component.html");
const styles = require("./feature-paginated-list-embed.component.scss");

export class FeaturePaginatedListEmbedComponent extends PaginatedComponent<Feature> {
    constructor(
    ...
    public render() {
        ...
        for (let i = 0; i < this.pagedList.data.length; i++) {            
            const el = this._document.createElement(`ce-feature-item-embed`);
...
customElements.define("ce-feature-paginated-list-embed", FeaturePaginatedListEmbedComponent);
