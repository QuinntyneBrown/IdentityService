...
import { IdentityService } from "./account.service";
...
export class AccountMasterDetailComponent extends HTMLElement {
    ...
    private async _bind() {
        this.accounts = await this._IdentityService.get();
    ...
    public async onAccountAdd(e) {
        ...
        await this._IdentityService.add(e.detail.account);
        this.accounts = await this._IdentityService.get();
    ...
    public async onAccountDelete(e) {        
        await this._IdentityService.remove({ id: e.detail.account.id });
        this.accounts = await this._IdentityService.get();
